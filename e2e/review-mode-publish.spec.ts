...
test('Review Mode publishes an edited generated draft and stores the GitHub issue URL', async () => {
  ...
  const [githubOwner, githubRepo] = githubRepoFullName!.split('/')
  test.skip(
  ...
  await page.evaluate(
    ...
    { repoPath: repoDir, owner: githubOwner!, repo: githubRepo!, token: githubToken! }
  )
  await page.reload()
  ...
  await expect
    ...
    .toMatchObject({
      ...
      githubIssueUrl: expect.stringMatching(
        new RegExp(
          `^https://github\\.com/${escapeRegExp(githubOwner!)}/${escapeRegExp(
            githubRepo!
          )}/issues/\\d+$`
        )
      )
