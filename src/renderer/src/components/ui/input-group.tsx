...
function InputGroupAddon({
  ...
  const focusInput = (target: HTMLElement, root: HTMLElement): void => {
    if (target.closest('button')) return
    root.parentElement?.querySelector('input')?.focus()
  }
  ...
  return (
    <div
      ...
      onClick={(e) => {
        focusInput(e.target as HTMLElement, e.currentTarget)
      }}
      onKeyDown={(e) => {
        if (e.key !== 'Enter' && e.key !== ' ') return
        e.preventDefault()
        focusInput(e.currentTarget, e.currentTarget)
      }}
      {...props}
    />
  )
