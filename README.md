...
PiLog is a **local-first Electron desktop app** for developers who want to **capture rough markdown notes on a hotkey** and **triage them in an inbox** before they become GitHub issues. Today the repo implements the capture pipeline (scratchpad, tray, SQLite persistence, typed IPC). Connecting GitHub and running the integrated Pi agent for draft issues is **planned**—see `docs/pilog_prd.md` and `docs/implementation-plan.md`.
